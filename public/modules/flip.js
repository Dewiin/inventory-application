...
cards.forEach((card) => {
	card.addEventListener("click", async () => {
        ...
        const frontCard = document.querySelector(`.pokemon-view.card-front[data-name="${name}"]`);
        const backCard = document.querySelector(`.pokemon-view.card-back[data-name="${name}"]`);

		const response = await fetch(`/api/pokemon/${name}/details`);
		const data = await response.json();

		const typesElement = document.querySelector(`.pokemon-view.card-back[data-name="${name}"] .types`);
