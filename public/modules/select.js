...
addPokemonCards.forEach((card) => {
  card.addEventListener("click", () => {
    ...
    const selectBox = document.querySelector(
      `.add-pokemon-view > div[data-name='${name}']`,
    );
    const checkbox = document.querySelector(
      `.add-pokemon-view[data-name='${name}'] > input[type="checkbox"]`,
    );

    const isAlreadySelected = selectBox.classList.contains("selected");

    let selectedPokemonCards = document.querySelectorAll(
      ".add-pokemon-view .selected",
    );
